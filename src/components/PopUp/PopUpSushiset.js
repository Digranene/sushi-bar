...
export const PopUpSet = () => {
    const classes = useStyles()
    const history = useHistory()
    const params = useParams()
    const id = +params.id
    const currentCardSet = sushiSet.find((s) => s.id === id)
    const handleClose = () => {
        history.push("/")
    }
        ...
        <Dialog open={true} onClose={handleClose}
            ...
            <Card className={classes.card}>
                ...
                <CardContent className={classes.cardContent}>
                    ...
                    <Typography>{currentCardSet.weight}</Typography>
                </CardContent>
                <CardActions>
                    <Typography variant="h5" color="primary">{`Price-${currentCardSet.price}`}</Typography>
                </CardActions>
            </Card>
            <DialogActions>
